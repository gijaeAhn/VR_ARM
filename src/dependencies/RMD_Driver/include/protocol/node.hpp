...
namespace rmd_driver {
    ...
    class Node: protected can::Node{
        ...
        */
        inline void send(std::uint32_t const actuator_id, Request const& request);
        ...
        */
        ...
        [[nodiscard]]
        inline RESPONSE_TYPE sendRecv(std::uint32_t const actuator_id, Request const& request);


        private:

        std::vector<std::uint32_t> actuator_id_list_;
        std::vector<std::uint32_t> can_send_id_list_;
        std::vector<std::uint32_t> can_receive_id_list_;
    ...
    void Node::updateIds(std::uint32_t const actuator_id) {
        if ((actuator_id < 1) || (actuator_id > 32)) {
            throw Exception("Given actuator id '" + std::to_string(actuator_id) + "' out of range [1, 32]!");
        }

        actuator_id_.push_back(actuator_id_list_);
        can_send_id_.push_back( id_offset_ + actuator_id_[actuator_id -1]) ;
        can_receive_id_.push_back( id_offset_ + actuator_id_[actuator_id -1]);
    ...
    template<typename RESPONSE_TYPE>
    Response Node::sendRecv(std::uint32_t const actuator_id, Request const& request) {
